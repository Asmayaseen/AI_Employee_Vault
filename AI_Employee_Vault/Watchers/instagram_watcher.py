"""
Instagram Watcher - Monitors Instagram DMs and mentions via Graph API.

Uses the social-mcp InstagramAdapter for API calls.
No browser automation needed - pure API-based.

Setup:
1. Instagram Business/Creator account connected to Facebook Page
2. Meta App with Instagram Graph API enabled
3. Set env vars: INSTAGRAM_BUSINESS_ID, META_ACCESS_TOKEN

Required scopes: instagram_basic, instagram_manage_messages
"""
...
class InstagramWatcher(BaseWatcher):
    """
    Watches Instagram DMs and mentions via Graph API.
    ...
    def __init__(self, vault_path: str = None, check_interval: int = 300):
        ...
        # Track processed IDs
        self.processed_ids_file = self.vault_path / '.processed_instagram'
        ...
        self._configured = bool(
            os.getenv('INSTAGRAM_BUSINESS_ID') and os.getenv('META_ACCESS_TOKEN')
        )
        if not self._configured:
            self.logger.warning(
                "Instagram not configured. Set INSTAGRAM_BUSINESS_ID and META_ACCESS_TOKEN in .env"
            )
    ...
    def _get_adapter(self):
        """Lazy-initialize the Instagram adapter."""
        if self._adapter is None:
            try:
                from adapters.instagram import InstagramAdapter
                self._adapter = InstagramAdapter()
            except ImportError:
                self.logger.error("Could not import InstagramAdapter from social-mcp")
                raise
    ...
    def check_for_updates(self) -> list:
        """Check Instagram for new DMs and mentions."""
        if not self._configured:
            return []
        ...
        try:
            ...
            for msg in messages:
                if msg.id not in self.processed_ids:
                    ...
                    self.logger.info(f"New Instagram DM from {msg.sender_name}")

            # Check mentions/comments
            notifications = self._run_async(
                adapter.fetch_notifications(types=['comment', 'mention'], limit=20)
            )
            for notif in notifications:
                if notif.id not in self.processed_ids:
                    new_items.append({
                        'item_type': 'social_mention' if notif.type == 'mention' else 'social_comment',
                        'id': notif.id,
                    ...
                    self.logger.info(f"New Instagram {notif.type} from {notif.actor_name}")
    ...
    def create_action_file(self, item: dict) -> Path:
        """Create action file for an Instagram DM or mention."""
        try:
...
---
*Created by InstagramWatcher*
'''

            filename = f"INSTAGRAM_{timestamp}_{safe_preview}.md"
            ...
            # Log action
            self.log_action('instagram_item_received', {
                'item_type': item_type,
                'message_id': item['id'],
                'from': sender_name,
                'priority': priority,
...
def test_watcher():
    """Test Instagram watcher (graceful skip if not configured)."""
    ...
    try:
        watcher = InstagramWatcher(vault_path=vault_path)

        if not watcher._configured:
            print("Instagram not configured - skipping API test")
            print("Set INSTAGRAM_BUSINESS_ID and META_ACCESS_TOKEN in .env")
...
if __name__ == '__main__':
    ...
    if args.test:
        ...
        print(f"Starting Instagram Watcher...")
